#!/usr/bin/env python
# 2D, No boundary traction, no x displacement on left or right, no y displacement on floor
# LM Dirichlet Pressure @ Hydrostatic on floor
# LM Dirichlet Pressure @ zero at surface
################################################################################
# NOTES:
# Modified LM Pressure to act from 0 to time t
#
...
t_dyn = 0.025; dt_dyn=1.6E-5; t_lim = 5.; dsp=1; dsp_hyb=0; dsp_str=1; rsf=0; v_bg=1E-12
...
# Quick fix to eliminate horizontal displacement on both edges in
# x direction
bcx_nodes_L = nc.variables['node_ns3'][:]
bcx_nodes_R = nc.variables['node_ns5'][:]

bc_typ = np.ones((len(coord),3), dtype=np.int8)
#for node in bcx_nodes:
#    bc_typ[node - 1, 0] = 0
for node in bcx_nodes_L:
    bc_typ[node - 1, 0] = 0
for node in bcx_nodes_R:
    bc_typ[node - 1, 0] = 0
...
surface_nodes = nc.variables['node_ns1']
...
if poro:
    if len(hydrostatic_press) > 0:
        for i in np.arange(0,len(hydrostatic_nodes)):
            ...
            line2 = [[hydrostatic_press[i], 0.0, t]]
